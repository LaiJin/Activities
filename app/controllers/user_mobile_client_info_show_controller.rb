class UserMobileClientInfoShowController < ApplicationController
  ...
  def activity_sign_up_list_view
    @activity_sign_ups = ActivitySignUp.where(:user_name => current_user.name, :activity_name => params[:activity_name]).paginate(page: params[:page], :per_page => PER_PAGE_COUNT)
    @count = USER_NUMBER_INIT
    if params[:page]
      @count = Integer(((Integer(params[:page]) - 1) * PER_PAGE_COUNT))
    end
